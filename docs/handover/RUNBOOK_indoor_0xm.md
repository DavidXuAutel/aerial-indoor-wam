...
1. ~~E2d~~ ✅；~~E2e/E2f~~ ❌；~~E2g~~ ⏸ **已停**（仍在室外 `env_airsim_16` 上采，场景不合格）。  
2. **进行中 = E2h（人令：首选推进）**：**换 AirSim 室内场景**（`Blocks` 冒烟 → `Building_99` 主采）→ 近障过门 → 自采短段 → FT → 合同评。  
3. **禁 E3**；禁把室外爬升/夹具室外到点写进完成态；禁关罩。

默认：**125 @ `/home/yao/aerial-indoor-wam`**；FT **4090 OK**。  
