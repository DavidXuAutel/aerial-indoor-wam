...
def main(argv: List[str] | None = None) -> int:
    ...
    p.add_argument(
        "--init-ckpt",
        default=None,
        help="warm-start from an existing wm_step_*.pt before indoor encode FT "
        "(E2i.A: fine-tune outdoor WM on indoor RGB windows)",
    )
    p.add_argument(
        "--skip-gate",
        action="store_true",
        help="always save ckpt after training steps even if LEARNING/NON-DIVERGENCE "
        "gate fails (indoor encode FT diagnostic; still logs pass/fail)",
    )
    ...
    if args.init_ckpt:
        ...
        if not init_path.is_file():
            # allow repo-relative
            cand = Path.cwd() / init_path
            if cand.is_file():
                init_path = cand
        if not init_path.is_file():
            print(f"[wm-validate] FAIL: --init-ckpt not found: {args.init_ckpt}", file=sys.stderr)
            return 1
        payload = model.load_checkpoint(str(init_path))
        print(
            f"[wm-validate] warm-start from {init_path} "
            f"step={payload.get('step')} skipped={len(payload.get('load_skipped') or [])} "
            f"missing={len(payload.get('load_missing') or [])}"
        )
    ...
    if (passed or args.skip_gate) and args.save_ckpt:
        ...
        print(f"[wm-validate] checkpoint → {ckpt_path}"
              + (" (saved despite gate FAIL; --skip-gate)" if not passed else ""))
