...
class ActorCriticConfig:
    ...
    #: When True, actor/critic input is ``concat(z, goal_rel[4])`` — required
    #: for imagination-to-goal navigation. Old goal-blind checkpoints cannot
    #: load into this mode.
...
class LatentActorCritic:
    ...
    def _feat_tensor(
        ...
        return torch.as_tensor(np.concatenate([z, g], axis=-1), device=self._device)
    ...
    def value(
        ...
        if z_arr.ndim == 3:
            # [B, T, D] — flatten time, optional goal_rel [B, T, 4]
            b, t, d = z_arr.shape
    ...
    def load_from_checkpoint(
        ...
        *,
        ...
        if "condition_on_goal" not in raw_cfg:
            # Pre-goal-conditioning checkpoints: keep input dim = latent_dim.
...
class LatentActorDeployPolicy:
    """Real-env policy: ``encode(obs)`` → ``act_latent(z, goal_rel)``."""
    ...
    def act(self, view: Any) -> np.ndarray:
        ...
        obs = Observation(rgb=view.rgb, state=state, t=float(view.t), info=info)
        if (
            self.stream_latent
        ...
        if goal is not None:
            gr = goal_rel_body(view.position, float(view.yaw), goal)
        else:
            gr = np.zeros(GOAL_REL_DIM, dtype=np.float32)
