...
def approach_bias_transition_episodes(
    episodes: Sequence[Sequence[Transition]],
    dist_m: float = 12.0,
) -> List[List[Transition]]:
    """Rewrite NPZ episode goals to ``start + dist · heading`` (indoor FT)."""
    dist = float(dist_m)
    if dist <= 0:
        raise ValueError(f"approach_dist_m must be > 0, got {dist_m}")
    ...
    for ep in episodes:
        if not ep:
            out.append(list(ep))
            continue
        start = np.asarray(ep[0].obs.position, dtype=np.float64).reshape(3)
        yaw0 = float(ep[0].obs.yaw)
        goal = start + dist * np.array(
            [math.cos(yaw0), math.sin(yaw0), 0.0], dtype=np.float64
        )
        stamped = list(ep)
        attach_goal(stamped, goal)
        out.append(stamped)
    return out
