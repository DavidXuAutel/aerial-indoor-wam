...
def _rank_b1(eps: List[Dict[str, Any]], src: Path, drop_early_coll: bool) -> List[Tuple[float, Path, Dict[str, Any]]]:
    ...
    for i, ep in enumerate(eps):
        ...
        if not fname:
            # indoor_loop_collect summary often omits file; episodes align by index
            cand = src / f"episode_{i:05d}.npz"
            fname = cand.name if cand.is_file() else None
        if not fname:
            continue
        path = src / fname
        if not path.is_file():
            continue
        if drop_early_coll and bool(ep.get("collided")):
            continue
        # Prefer summary collided flag; npz early-coll is noisy on indoor near-field
        # (transient contact bits). Only use npz probe when summary lacks collided.
        if drop_early_coll and "collided" not in ep and _early_collision(path):
            continue
...
def main() -> int:
    ...
    ap.add_argument("--b2", default="experiments/aerial/rl/artifacts/dataset_indoor_b99_fixture_020_20260831")
