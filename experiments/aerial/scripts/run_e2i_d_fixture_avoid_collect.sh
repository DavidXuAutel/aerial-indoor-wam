...
# avoid JSON idx: 4=NE lateral (proven); 5=R06 east corridor; 6=east; 7=south; 8=east_from1
ROUTES="${ROUTES:-4,4,4,4,4,4,5,5,5,6,6,7,7,8,8}"
EPISODES="${EPISODES:-80}"
MIN_USABLE="${MIN_USABLE:-15}"
MIN_USABLE_NEW="${MIN_USABLE_NEW:-8}"
MIN_R06="${MIN_R06:-8}"
APPEND="${APPEND:-0}"
...
echo "[d1] fixture avoid gt_pd_body out=$OUT routes=$ROUTES append=$APPEND success<=${SUCCESS_DIST}m" | tee "$LOG"

APPEND_ARGS=()
if [[ "$APPEND" == "1" ]]; then
  APPEND_ARGS+=(--append --min-usable-new "$MIN_USABLE_NEW")
fi
