#!/usr/bin/env bash
# E3 odom eval @0.50 on clean_sg (F-cap gates). BASELINE=1 uses E head pre-FT.
set -euo pipefail
...
if [[ "${BASELINE:-0}" == "1" ]]; then
  TAG=e3_odom_baseline_050
  PROTOCOL=e3_odom_baseline_cap
  ACT="${ACT:-experiments/aerial/rl/artifacts/v4_ac_ckpt_indoor_e2i_e_20260901/v4_ac_latest.pt}"
else
  TAG=e3_odom_cap_050
  PROTOCOL=e3_odom_cap
  ACT="${ACT:-experiments/aerial/rl/artifacts/v4_ac_ckpt_indoor_e3_odom_${STAMP}/v4_ac_latest.pt}"
fi
...
: >"$LOG"
echo "[$TAG] odom eval @0.50 act=$ACT routes=$ROUTES $(date -Is)" | tee -a "$LOG"
