#!/usr/bin/env bash
# E2i.F F1 — primary gate: @0.50 nospawn (drop R01 + R06). No FT.
# Contract: docs/handover/INDOOR_E2I_F_PLAN_20260901.md
set -euo pipefail
...
: >"$LOG"
{
  echo "[f1] primary nospawn success_dist=$SUCCESS_DIST act=$ACT routes=$ROUTES $(date -Is)"
  echo "[f1] protocol=e2i_f_primary pose=gt_proxy(probe) assist=none shield_v3"
} | tee -a "$LOG"
...
out = {
    "protocol": "e2i_f_primary",
    "contract": "INDOOR_E2I_F_PLAN_20260901.md",
    "success_dist_m": succ,
    ...
    "stretch_note": "0.20 remains stretch-only; see E4b / SUCCESS_DIST=0.20",
    "seeds": rows,
}
...
echo "[f1] done $(date -Is)" | tee -a "$LOG"
